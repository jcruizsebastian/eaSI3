...
namespace JiraConnector
{
    public class JiraURIRepository
    {
        ...
        public static string GET_WORKLOG(DateTime startDate, DateTime endDate, string username, string project) => $"rest/api/2/search?jql=issueFunction in workLogged(\"{(username != null ? $"by {username} " : "" )} after {startDate.ToString("yyyy/MM/dd")} before {endDate.ToString("yyyy/MM/dd")}\") {(project != null ? $"and project in (\"{project}\")" : "")}";
