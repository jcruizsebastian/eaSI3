...
namespace SI3Connector
{
    public class SI3Service
    {
        ...
        public SI3Service(string username, string passworkd)
        {
        ...
        public async Task<string> Login()
        {
            ...
            request.Content = new StringContent($"user={username}&pwd={password}&DSN=GESOPENFINANCE");
        ...
        public async Task<string> AddWorklog(string issueid, DateTime date, int time)
        {
            ...
            request.Headers.Add("Accept-Language", "es-ES,es;q=0.5");
            request.Headers.TryAddWithoutValidation("Content-Type", "application/x-www-form-urlencoded");
            request.Content = new StringContent($"newTimeRecord={time}&newDate={date.Day.ToString("D2")}%2F{date.Month.ToString("D2")}%2F{date.Year - 2000}&timerecordtype=R");
